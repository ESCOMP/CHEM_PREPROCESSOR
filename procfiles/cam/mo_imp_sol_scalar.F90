...
!!$      if ( reduced_ozone_chem .or. full_ozone_chem ) then
!!$
!!$         call addfld( 'PO3CHM', '/cm3/s ', pver, 'I', 'net tendency from chem', phys_decomp )
!!$         call add_default( 'PO3CHM', 1, ' ' )
!!$         call addfld( 'LO3CHM', '/cm3/s ', pver, 'I', 'net tendency from chem', phys_decomp )
!!$         call add_default( 'LO3CHM', 1, ' ' )
!!$
!!$      endif
      do i = 1,clscnt4
         j = clsmap(i,4)
...
!!$       call add_default( trim(solsym(j))//'_CHMP', 1, ' ' )
...
!!$      real(r8), dimension(ncol,pver) :: po3,lo3
!!$
!!$      po3 = 0._r8
!!$      lo3 = 0._r8
      prod_out(:,:,:) = 0._r8
...
!!$               has_o3_chem: if( full_ozone_chem .or. reduced_ozone_chem ) then
               has_o3_chem: if( ( full_ozone_chem .or. reduced_ozone_chem ) .and. j == ox_ndx  ) then
                  if( o1d_ndx < 1 ) then
!!$                     lo3(i,lev) = reaction_rates(i,lev,ox_l1_ndx)
                     loss_out(i,lev,j) = reaction_rates(i,lev,ox_l1_ndx)
                  else
!!$                     lo3(i,lev) = reaction_rates(i,lev,ox_l1_ndx) * base_sol(i,lev,o1d_ndx)/base_sol(i,lev,ox_ndx)
                     loss_out(i,lev,j) = reaction_rates(i,lev,ox_l1_ndx) * base_sol(i,lev,o1d_ndx)/base_sol(i,lev,ox_ndx)
                  end if
                  if ( full_ozone_chem ) then
!!$                 po3(i,lev) = reaction_rates(i,lev,ox_p1_ndx)  * base_sol(i,lev,ho2_ndx) &
...
!!$              po3(i,lev) = po3(i,lev) * base_sol(i,lev,no_ndx)
!!$              lo3(i,lev) = lo3(i,lev) * base_sol(i,lev,ox_ndx)
...
!!$         po3 = po3 * xhnm
!!$         lo3 = lo3 * xhnm
!!$         call outfld( 'PO3CHM', po3(:,:), ncol, lchnk )
!!$         call outfld( 'LO3CHM', lo3(:,:), ncol, lchnk )
!!$      endif
      do i = 1,clscnt4
